...
def upgrade() -> None:
    ...
    # First, update existing companies table to handle the subscription_status column
    # Convert existing VARCHAR values to match enum values
    connection.execute("""
        UPDATE companies 
    ...
    op.execute("""
        ...
        USING subscription_status::text::subscriptionstatus
    """)
...
def downgrade() -> None:
    ...
    op.execute("UPDATE companies SET subscription_status = 'trial' WHERE subscription_status = 'TRIAL'")
    ...
    # Drop enum types
    connection = op.get_bind()
    connection.execute("DROP TYPE IF EXISTS subscriptionstatus")
    connection.execute("DROP TYPE IF EXISTS usagemetrictype")
    connection.execute("DROP TYPE IF EXISTS billingplantype")
    connection.execute("DROP TYPE IF EXISTS auditactiontype")
